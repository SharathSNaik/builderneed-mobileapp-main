...
if ($count == 1) {
    ...
    $role = $rows['user_role'];
    if ($role == 1) {
        $userquery = mysqli_query($connection, "SELECT * FROM bn_user_activity WHERE pr_id = '$pid'");
    } else {
        $userquery = mysqli_query($connection, "SELECT * FROM bn_user_activity WHERE pr_id = '$pid' AND agent_id='$id'");
    }
    $c = 0;
